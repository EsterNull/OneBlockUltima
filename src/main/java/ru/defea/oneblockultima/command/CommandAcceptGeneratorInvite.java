...
public final class CommandAcceptGeneratorInvite
{
    public static void register(CommandDispatcher<CommandSourceStack> dispatcher)
    {
        dispatcher.register(Commands.literal("acceptGeneratorInvite")
    ...
    private static void execute(CommandSourceStack source)
    {
        ...
        if (generator.isMemberLimitReached() && !generator.hasAccess(player.getUUID()))
        {
            source.sendFailure(Component.translatable("command.inviteGeneratorMember.member_limit").withStyle(ChatFormatting.RED));
            return;
        }

        if (!generator.acceptInvite(player.getUUID()))
        {
            source.sendFailure(Component.translatable("command.no_invite").withStyle(ChatFormatting.RED));
            return;
        }

        source.sendSuccess(() -> Component.translatable("command.acceptGeneratorInvite.accepted").withStyle(ChatFormatting.GREEN), false);
    }
}
